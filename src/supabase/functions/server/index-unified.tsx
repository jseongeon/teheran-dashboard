...
// 📌 버전 정보
// ========================================
const SYNC_VERSION = "1.2.5";
const LAST_UPDATE = "2025-01-20T00:00:00Z";
...
interface InquiryRow {
  ...
  isExcluded: boolean;
  isDuplicate: boolean;
  originalRowNumber: number;  // ✅ 추가: 원본 행 번호
}
...
function transformSheetRowToInquiry(row: any[], rowIndex: number): InquiryRow | null {
  ...
  return {
    ...
    isExcluded,
    isDuplicate,
    originalRowNumber: rowIndex,  // ✅ 추가: 원본 행 번호
  };
}
...
app.post('/make-server-1da81fff/api/sync-to-db', async (c) => {
  ...
  try {
    ...
    // 2. 데이터 변환
    // ✅ 수정: B5:S 범위로 가져오므로 실제 행 번호는 5 + index
    console.log("🔄 데이터 변환 중...");
    let inquiries = rows
      .map((row, index) => transformSheetRowToInquiry(row, 5 + index))
...
app.post('/make-server-1da81fff/api/sync-to-db-range', async (c) => {
  ...
  try {
    ...
    // 데이터 변환 (행 번호 계산 주의)
    // ✅ 수정: B${startRow}:S${endRow} 범위이므로 실제 행 번호는 startRow + index
    let inquiries = rows
      .map((row, index) => transformSheetRowToInquiry(row, startRow + index))
      .filter((inquiry): inquiry is NonNullable<typeof inquiry> => inquiry !== null);
